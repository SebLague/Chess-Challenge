...
namespace ChessChallenge.Framework.Application.Helpers
{
    public static class Tester
    {
        ...
        public static void RunPerft(bool useStackalloc = true)
        {
            ...
            var board = new ChessEngine.Internal.Board.Board();
            long timeTotal = 0;
        ...
        static void CheckTest()
        {
            ...
            Move move = new Move("d3h7", boardAPI);
            boardAPI.MakeMove(move);
            Assert(boardAPI.IsInCheck(), "Check wrong");
            boardAPI.UndoMove(move);
            Assert(!boardAPI.IsInCheck(), "Check wrong");

            boardAPI.MakeMove(new Move("f3h5", boardAPI));
            boardAPI.MakeMove(new Move("f6d7", boardAPI));
            Assert(!boardAPI.IsInCheckmate(), "Checkmate wrong");
            boardAPI.MakeMove(new Move("h5h7", boardAPI));
            Assert(boardAPI.IsInCheckmate(), "Checkmate wrong");
        ...
        static void PieceListTest()
        {
            ...
            PieceList[] pieceLists = boardAPI.GetAllPieceLists();
            ...
            Move testMove = new("g5e6", boardAPI);
        ...
        static void RepetitionTest()
        {
            ...
            {
                var move = new Move(name, boardAPI);
                boardAPI.MakeMove(move);
                return move;
            }
        }
        ...
        static void DrawTest()
        {
            ...
            boardAPI.MakeMove(new Move("c7g7", boardAPI));
            Assert(!boardAPI.IsDraw(), "Draw wrong");
            boardAPI.MakeMove(new Move("h6h4", boardAPI));
            Assert(!boardAPI.IsDraw(), "Draw wrong");
            boardAPI.MakeMove(new Move("g7c7", boardAPI));
            Assert(!boardAPI.IsDraw(), "Draw wrong");
            boardAPI.MakeMove(new Move("h4h6", boardAPI));
            Assert(boardAPI.IsDraw(), "Draw wrong");
            boardAPI.UndoMove(new Move("h4h6", boardAPI));
            Assert(!boardAPI.IsDraw(), "Draw wrong");

            // Stalemate test
            board = new ChessEngine.Internal.Board.Board();
            board.LoadPosition("7K/8/6k1/5q2/8/8/8/8 b - - 0 1");
            boardAPI = new(board);
            Assert(!boardAPI.IsDraw(), "Draw wrong");
            boardAPI.MakeMove(new Move("f5f7", boardAPI));
            Assert(boardAPI.IsDraw(), "Draw wrong");
            // Test stalemate bug when generating captures
            board.LoadPosition("8/8/3k4/8/8/3K4/4Q3/8 w - - 0 1");
            boardAPI = new Board(board);
            ...
            board.LoadPosition("8/8/3k4/8/8/3K4/4Q3/8 w - - 0 1");
            boardAPI = new Board(board);
            Span<Move> captureSpan = stackalloc Move[64];
            ...
            {
                boardAPI = Board.CreateBoardFromFEN(drawPos);
                Assert(boardAPI.IsDraw(), "Draw wrong, position is insufficient mat");
                boardAPI = Board.CreateBoardFromFEN(FenUtility.FlipFen(drawPos));
                Assert(boardAPI.IsDraw(), "Draw wrong, position is insufficient mat");
            }

            foreach (string winnablePos in notInsufficient)
            {
                boardAPI = Board.CreateBoardFromFEN(winnablePos);
                Assert(!boardAPI.IsDraw(), "Draw wrong, position is winnable");
                boardAPI = Board.CreateBoardFromFEN(FenUtility.FlipFen(winnablePos));
                Assert(!boardAPI.IsDraw(), "Draw wrong, position is winnable");
            }
        }
        ...
        static void MiscTest()
        {
            ...
            var b = new ChessEngine.Internal.Board.Board(board);
            boardAPI = new(b);
        ...
        static void MoveGenTest()
        {
            ...
            ChessEngine.Internal.Board.Board board = new();
            moveGen = new();
        ...
        public class SearchTest3
        {
            ...
            void Search(int depth, Move prevMove)
            {
