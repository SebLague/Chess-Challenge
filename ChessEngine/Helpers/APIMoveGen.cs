...
using ChessEngine.Internal.MoveGeneration.Bitboards;
using ChessEngine.Internal.MoveGeneration.Magics;
using static ChessEngine.Internal.MoveGeneration.PrecomputedMoveData;
...
namespace ChessEngine.Helpers
{
    ...
    public class APIMoveGen
    {
        ...
        public APIMoveGen()
        {
            board = new Internal.Board.Board();
        }
        ...
        public ulong GetOpponentAttackMap(Internal.Board.Board board)
        {
            Init(board);
            return opponentAttackMap;
        }

        public bool NoLegalMovesInPosition(Internal.Board.Board board)
        {
        ...
        public void GenerateMoves(ref Span<Move> moves, Internal.Board.Board board, bool includeQuietMoves = true)
        {
        ...
        public void Init(Internal.Board.Board board)
        {
            this.board = board;
            currMoveIndex = 0;
        ...
        Move CreateAPIMove(int startSquare, int targetSquare, int flag)
        {
        ...
        Move CreateAPIMove(int startSquare, int targetSquare, int flag, int movePieceType)
        {
            int capturePieceType = PieceHelper.PieceType(board.Square[targetSquare]);
            if (flag == Internal.Board.Move.EnPassantCaptureFlag)
            {
                capturePieceType = PieceHelper.Pawn;
            }
            Move apiMove = new(new Internal.Board.Move(startSquare, targetSquare, flag), movePieceType, capturePieceType);
            return apiMove;
        }
            ...
            if (!inCheck && generateNonCapture)
            {
                ...
                if (board.currentGameState.HasKingsideCastleRight(board.IsWhiteToMove))
                {
                    ...
                    if ((castleMask & castleBlockers) == 0)
                    {
                        ...
                        moves[currMoveIndex++] = CreateAPIMove(friendlyKingSquare, targetSquare, Internal.Board.Move.CastleFlag, PieceHelper.King);
                    }
                }
        ...
        void GenerateSlidingMoves(Span<Move> moves, bool exitEarly = false)
        {
        ...
        void GeneratePawnMoves(Span<Move> moves)
        {
            ...
            if (generateNonCapture)
            {
                ...
                while (doublePush != 0)
                {
                    ...
                    if (!IsPinned(startSquare) || alignMask[startSquare, friendlyKingSquare] == alignMask[targetSquare, friendlyKingSquare])
                    {
                        moves[currMoveIndex++] = CreateAPIMove(startSquare, targetSquare, Internal.Board.Move.PawnTwoUpFlag, PieceHelper.Pawn);
                    }
                }
            }
                ...
                if (BitBoardUtility.ContainsSquare(checkRayBitmask, capturedPawnSquare))
                {
                    ...
                    while (pawnsThatCanCaptureEp != 0)
                    {
                        ...
                        if (!IsPinned(startSquare) || alignMask[startSquare, friendlyKingSquare] == alignMask[targetSquare, friendlyKingSquare])
                        {
                            ...
                            {
                                moves[currMoveIndex++] = CreateAPIMove(startSquare, targetSquare, Internal.Board.Move.EnPassantCaptureFlag, PieceHelper.Pawn);
                            }
                        }
                    }
                }
        ...
        void GeneratePromotions(Span<Move> moves, int startSquare, int targetSquare)
        {
            moves[currMoveIndex++] = CreateAPIMove(startSquare, targetSquare, Internal.Board.Move.PromoteToQueenFlag, PieceHelper.Pawn);
            // Don't generate non-queen promotions in q-search
            if (generateNonCapture)
            {
                if (promotionsToGenerate == MoveGenerator.PromotionMode.All)
                {
                    moves[currMoveIndex++] = CreateAPIMove(startSquare, targetSquare, Internal.Board.Move.PromoteToKnightFlag, PieceHelper.Pawn);
                    moves[currMoveIndex++] = CreateAPIMove(startSquare, targetSquare, Internal.Board.Move.PromoteToRookFlag, PieceHelper.Pawn);
                    moves[currMoveIndex++] = CreateAPIMove(startSquare, targetSquare, Internal.Board.Move.PromoteToBishopFlag, PieceHelper.Pawn);
                }
