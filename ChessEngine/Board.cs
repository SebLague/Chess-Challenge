...
namespace ChessEngine
{
	...
	{
		readonly Internal.Board.Board board;
        ...
        /// </summary>
        public Board(Internal.Board.Board boardSource)
		{
			...
			{
				Internal.Board.Move move = boardSource.AllGameMoves[i];
		...
		public void MakeMove(Move move)
		{
			...
			{
				OnPositionChanged();
				board.MakeMove(new Internal.Board.Move(move.RawValue), inSearch: true);
		...
		public void UndoMove(Move move)
		{
			if (!move.IsNull)
			{
				...
				board.UndoMove(new Internal.Board.Move(move.RawValue), inSearch: true);
                OnPositionChanged();
				depth--;
			}
		}
		...
		{
			int colIndex = white ? Internal.Board.Board.WhiteIndex : Internal.Board.Board.BlackIndex;
		...
		/// </summary>
		public ulong WhitePiecesBitboard => board.colourBitboards[Internal.Board.Board.WhiteIndex];
		/// <summary>
		/// 64-bit number where each bit that is set to 1 represents a square that contains any type of black piece.
		/// </summary>
		public ulong BlackPiecesBitboard => board.colourBitboards[Internal.Board.Board.BlackIndex];
        ...
        {
            Internal.Board.Board boardCore = new Internal.Board.Board();
