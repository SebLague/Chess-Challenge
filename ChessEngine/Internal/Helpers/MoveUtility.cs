...
namespace ChessEngine.Internal.Helpers
{
	...
	public static class MoveUtility
	{
		...
		public static Board.Move GetMoveFromUCIName(string moveName, Internal.Board.Board board)
		{
			...
			if (movedPieceType == PieceHelper.Pawn)
			{
				...
				if (moveName.Length > 4)
				{
					...
					{
						'q' => Board.Move.PromoteToQueenFlag,
						'r' => Board.Move.PromoteToRookFlag,
						'n' => Board.Move.PromoteToKnightFlag,
						'b' => Board.Move.PromoteToBishopFlag,
						_ => Board.Move.NoFlag
					};
				}
				// Double pawn push
			...
			else if (movedPieceType == PieceHelper.King)
			{
				...
				{
					flag = Board.Move.CastleFlag;
				}
			}

			return new Board.Move(startSquare, targetSquare, flag);
		}

		// Get name of move in UCI format
		// Examples: "e2e4", "e7e8q"
		public static string GetMoveNameUCI(Board.Move move)
		{
			if (move.IsNull)
			{
				return "Null";
			...
			if (move.IsPromotion)
			{
				switch (move.MoveFlag)
				{
					...
					case Board.Move.PromoteToKnightFlag:
						moveName += "n";
						break;
					case Board.Move.PromoteToBishopFlag:
						moveName += "b";
						break;
					case Board.Move.PromoteToQueenFlag:
						moveName += "q";
						break;
				}
			}
		...
		public static string GetMoveNameSAN(Internal.Board.Move move, Internal.Board.Board board)
		{
			if (move.IsNull)
			{
				return "Null";
			...
			if (move.MoveFlag == Board.Move.CastleFlag)
			{
			...
			if (movePieceType != PieceHelper.Pawn && movePieceType != PieceHelper.King)
			{
				...
				foreach (var altMove in allMoves)
				{
					...
					{ // if moving to same square from different square
						...
						{ // same piece type
							int fromFileIndex = BoardHelper.FileIndex(altMove.StartSquareIndex);
							int alternateFromFileIndex = BoardHelper.FileIndex(altMove.StartSquareIndex);
							int fromRankIndex = BoardHelper.RankIndex(altMove.StartSquareIndex);
			...
			{ // check if capturing ep
				if (move.MoveFlag == Board.Move.EnPassantCaptureFlag)
				{
		...
		public static Board.Move GetMoveFromSAN(Internal.Board.Board board, string algebraicMove)
		{
			...
			foreach (Board.Move moveToTest in allMoves)
			{
				move = moveToTest;
